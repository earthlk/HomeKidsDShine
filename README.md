...
**สถานะ:** Phase 5 — เพิ่มบันทึกกิจกรรมแล้ว
...
js/people.js            หน้าเด็กและผู้ปกครอง
js/courses.js           หน้าคอร์สและการลงทะเบียน
js/calendar.js          หน้าตารางนัด
js/trainers.js          หน้าผู้ฝึกสอน
js/users.js             หน้าผู้ใช้งาน
js/activity.js          หน้าบันทึกกิจกรรม
...
├── People.gs           เด็กและผู้ปกครอง
├── Courses.gs          คอร์สและการลงทะเบียน
├── Sessions.gs         ตารางนัด
├── Trainers.gs         ผู้ฝึกสอน
├── Users.gs            บัญชีผู้ใช้งาน
├── Activities.gs       บันทึกกิจกรรม
...
สร้างไฟล์ทุกตัวตามชื่อในโฟลเดอร์ `apps-script/` แล้ววางเนื้อหาลงไป
...
หน้าเด็กและผู้ปกครอง ผู้ฝึกสอน ผู้ใช้งาน คอร์ส และตารางนัด ใช้งานได้เต็มรูปแบบแล้ว
หน้าอื่นจะขึ้นว่า "อยู่ระหว่างพัฒนา" ซึ่งถูกต้องตามแผน
...
7. เพิ่มหน้าใหม่โดยลงทะเบียนใน `PAGES` ของ `app.js` และ `ROUTES` ของ `Main.gs`
   ไม่แก้ไฟล์อื่น
8. ปุ่มที่ต้องรอหลังบ้านตอบต้องใส่ `data-busy="ข้อความ"` เสมอ
   `bindSheet` จะปิดปุ่มและแสดงตัวหมุนให้เอง กันผู้ใช้กดซ้ำจนได้ข้อมูลซ้ำสองแถว
9. เพิ่มคอลัมน์ใน `SCHEMA` ได้เฉพาะต่อท้ายสุด ห้ามแทรกกลาง
   เพราะ `insertRow` และ `updateRow` เขียนตามลำดับใน `SCHEMA`
   ถ้าลำดับไม่ตรงกับหัวตารางในชีต ข้อมูลจะเขียนผิดคอลัมน์ทั้งแถว
   หลังเพิ่มแล้วต้องรัน `repairSheets()` เพื่อเติมคอลัมน์ให้ชีตที่มีข้อมูลอยู่แล้ว

---

## ตรวจก่อนส่งงาน

รัน `python3 tools-check.py` ทุกครั้งหลังแก้โค้ด ตรวจหกอย่าง:

1. เมธอดชื่อซ้ำในไฟล์หน้าจอ — ตัวหลังทับตัวแรกเงียบ ๆ
2. ฟังก์ชันชื่อซ้ำในหลังบ้าน รวมถึงซ้ำข้ามไฟล์ เพราะ Apps Script รวมทุกไฟล์เป็นขอบเขตเดียว
3. ตัวเลือก CSS ที่ประกาศซ้ำ
4. เส้นทางใน `ROUTES` ที่ชี้ไปหาฟังก์ชันที่ไม่มีอยู่จริง
5. หน้าจอเรียก action ที่ไม่มีในตารางเส้นทาง
6. หน้าที่อยู่ในเมนูแต่ยังไม่ได้ลงทะเบียนใน `PAGES`
